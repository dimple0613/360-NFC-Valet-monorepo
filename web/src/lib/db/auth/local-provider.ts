...
import { buildPasswordResetEmail, buildVerificationEmail, resolveAppBaseUrl, resolveEmailBranding } from "./email-templates";
  ...
  const template = buildVerificationEmail(
    {
      name: input.name,
      token,
      verifyUrl: `${resolveAppBaseUrl()}/verify-email?token=${encodeURIComponent(token)}`,
    },
    branding,
  );
  ...
  const template = buildPasswordResetEmail(
    {
      ...
      token,
      resetUrl: `${resolveAppBaseUrl()}/reset-password?token=${encodeURIComponent(token)}`,
    },
    branding,
  );
  await emailSender.send({
