...
## D-006: Valet /api/v1 endpoints — scoped to Phase 2 / module boundary (audit #22)

**Decision:** The API-key "Create" dialog shows only `core.*` scopes. Valet-domain permissions (`valet.card.*`, `valet.driver.*`, `valet.offer.*`, `valet.property.*`, `valet.queue.*`, `valet.reports.*`, `valet.dashboard.read`) remain in the permission registry (they gate internal tenant-admin pages and server actions) but are intentionally excluded from the API-key scope picker until real `/api/v1` routes back them. Granting a valet scope to an API key today yields a credential that can call nothing; filtering the picker removes the mismatch.

**Why:** The valet API surface is business-domain functionality and belongs behind the module/plugin boundary (`REQUIREMENTS.md` core = zero business logic). Building real `/api/v1` valet endpoints is Phase 2+ scope; showing them now creates a broken UX (keys scoped to unreachable routes) with no security benefit (the internal routes already enforce permission checks via session auth). D-006 keeps the contract between the dialog and the runtime surface honest without blocking the core API-key story.

**Does not lock in:** Where the valet /api/v1 surface lands (a "valet" module, a plugin, or core-backed shared endpoints behind a business boundary) — the permission registry and the dialog filter are pure UI data, and real valet routes are a single `LIKE 'valet.%'` filter removal away when they arrive.
