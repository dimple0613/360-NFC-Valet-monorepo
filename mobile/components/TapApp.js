...
function uidFromQr(text) {
  const t = String(text || "").trim();
  if (!t) return null;
  try {
    const asUrl = new URL(t, "https://location.invalid");
    const queried = asUrl.searchParams.get("uid") || asUrl.searchParams.get("id");
    if (queried) return queried.trim();
  } catch {}
  const last = t
    .split("/")
    .filter(Boolean)
    .pop();
  const seg = (last ? decodeURIComponent(last) : "").trim();
...
function Landing({ onNavigate }) {
  ...
  const startQrScan = async () => {
    ...
    if (!Detector || !navigator.mediaDevices?.getUserMedia) {
      setQrError("QR scanning needs a Chromium browser (Chrome / Edge) with a camera.");
      return;
    }
    setQrError("");
    try {
      ...
      const loop = async () => {
        ...
        try {
          ...
          if (codes && codes.length) {
            const uid = uidFromQr(codes[0].rawValue);
            if (uid) {
              stopQrScan();
              onNavigate(uid);
              return;
      ...
      <main className="page-content">
        <div className="panel" style={{ padding: "24px 20px", marginTop: 16 }}>
          <div className="nfc-scan-area">
            ...
            <button
              type="button"
              className={qrScanning ? "btn-nfc-active" : "btn-outline"}
              style={{ width: "100%" }}
              onClick={qrScanning ? stopQrScan : startQrScan}
            >
              {qrScanning ? "Scanning… point at the code" : "Scan card QR code"}
            </button>
            <div className="qr-scan-sub">NFC acting up? Scan the QR printed on the card instead.</div>
            {qrScanning && (
              <div className="qr-scan-video-wrap">
                <video ref={videoRef} className="qr-scan-video" playsInline muted />
              </div>
            )}
            {qrError && <div className="nfc-err">{qrError}</div>}
          </div>
        </div>
      </main>
...
function TapApp() {
  ...
  const slugMismatch = loadState === "ready" && slug && data?.property?.slug && slug !== data.property.slug;
  ...
  if (loadState === "landing") return <Landing onNavigate={(uid) => router.push({ pathname: "/", query: { uid } })} />;
  ...
  return (
    <div className="page">
      {slugMismatch && (
        <div className="slug-mismatch">
          <b>This card belongs to {data?.property?.name}</b>
          <span>You opened it from “{slug}” — check you printed and paired it correctly.</span>
        </div>
      )}
      {wsBanner && (
