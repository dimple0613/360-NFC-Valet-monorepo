...
- Surface the property's offers (featured first, category-filtered) with optional images and menu links, and let staff validate valet-parking offers with a 4-digit code.
...
- **Home (C1)** — property banner (with optional image), "Bring my car" hero, car strip (card, plate, zone), category grid, featured offers row (with optional images). If the order is returned, shows a "Visit complete" panel.
- **Bring my car (ETA sheet)** — bottom sheet with a 5–30 min stepper + preset chips; POSTs to the public tap API. Auto-opens when an active/parked order exists and no request is in progress.
...
- **Offers (C5)** — category listing with All / Offers-only / Open-now filters; optional offer images.
- **Offer detail (C6)** — price, save %, hours, optional menu link ("View menu"), staff validation code entry, "Call to reserve".
- **WebSocket banners** — parking / request-received / arrived / delay events show as live toast banners (requires `NEXT_PUBLIC_WS_URL`).
- **Slug-based routing** — supports `/<slug>/<uid>` URLs for property-specific card pages.
...
```
pages/          thin page wrappers (all render <TapApp />); includes [slug]/[uid] route
components/     TapApp.js — the entire guest app (views, state, sockets)
lib/            client.js — browser fetch wrapper (api(), adminUrl())
styles/         design system (globals.css)
```

- **Pages** only import and render `TapApp`; the UID is read from the URL (`/<slug>/<uid>` or `/?uid=<uid>`).
- **`lib/client.js`** is the only place `fetch` is called — it talks to the **admin console's** public API (`NEXT_PUBLIC_ADMIN_API`, default `http://localhost:3000/api`). It also exports `adminUrl(path)` for constructing full admin API URLs.
...
| Home (C1) | ✅ | Property banner (optional image), bring-my-car hero, car strip, categories, featured offers |
| Visit complete state | ✅ | Shown when order is returned; reload or browse offers |
| ETA sheet | ✅ | 5–30 min stepper + preset chips; auto-opens for active/parked orders |
| Live status (C3) | ✅ | Countdown ring, driver chip, 4-step timeline |
| Ready (C4) | ✅ | Green ready screen with pickup location |
| Offers listing (C5) | ✅ | All / Offers only / Open now filters; optional images |
| Offer detail (C6) | ✅ | Price, save %, hours, optional menu link, staff validation, call to reserve |
| Error / empty states | ✅ | Spinner, no-car panel, card-not-recognised |
| Slug-based routing | ✅ | `/<slug>/<uid>` route for property-specific pages |

### API consumption

| Requirement | Status | Notes |
|---|---|---|
| `GET /public/tap/[uid]` | ✅ | Resolves card → property (with optional imageUrl), order, offers (with optional imageUrl/menuUrl) |
| `POST /public/tap/[uid]` | ✅ | Bring-my-car (ETA) |
| `POST /public/offer/validate` | ✅ | Staff-code validation (offer-level and property-level) |
| Real-time WebSocket | ✅ | socket.io-client against `NEXT_PUBLIC_WS_URL`; banners on order events |
...
| Property/offer images | ✅ | `imageUrl` on property and offers; gradient fallback when absent |
| Offer menu link | ✅ | `menuUrl` on offers; "View menu" button on detail view |
...
3. **The WebSocket server is not part of this repo** — it runs separately; its URL is configured via `NEXT_PUBLIC_WS_URL` (see `docs/DEPLOYMENT.md`).
4. **Offer-level staff validation is hidden** — the offer detail view wraps the validation box in `display: "none"`, so only the property-level validation box (on home/status) is visible to guests.