...
| `image_url` | text nullable | property banner image (used in C1 banner when present) |
...
|---|---|---|
| `id` | serial PK | |
| `property_id` | int FK → properties | returned in tap response as `propertyId` |
| `card_id` | int FK → nfc_cards | |
...
| `image_url` | text nullable | offer card image (gradient fallback when absent) |
| `menu_url` | text nullable | link to offer menu; `data:` URIs resolved via `adminUrl(/public/offer/:id/menu)` |
...
- **Tap lookup** — `nfc_cards` by `uid` → its `property` (with optional `imageUrl`) + latest `active`/`parked`/`retrieving`/`returning` order (with `propertyId` and assigned driver) + live, non-draft `offers` for that property (with optional `imageUrl`/`menuUrl`, featured first).
