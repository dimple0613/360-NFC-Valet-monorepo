...
- ✅ **2. Card lookup** — UID from the URL (`/<slug>/<uid>`), resolve via `GET /api/public/tap/[uid]`.
...
- ✅ **11. WebSocket banners** — socket.io via `NEXT_PUBLIC_WS_URL`; parked / request-received / arrived / delay events with auto-refetch.
- ✅ **12. Property/offer images** — `imageUrl` on property and offers with gradient fallback; `menuUrl` for offer menu links.
- ✅ **13. Slug-based routing** — `/<slug>/<uid>` route for property-specific card pages.
- ⚠️ **14. Automated tests** — strategy documented in `docs/TESTING_STRATEGY.md`; not yet implemented.
- ⚠️ **15. Error/empty polish** — "no car found" panel, "card not recognised", and "visit complete" panel exist; more retry paths planned.
...
3. Set up the WebSocket server on a stable host and set `NEXT_PUBLIC_WS_URL` in production.