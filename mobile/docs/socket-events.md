...
| --- | --- | --- |
| Guest web (mobile) | `NEXT_PUBLIC_WS_URL` (socket.io, `transports: ["websocket","polling"]`) | anonymous |
...
Drivers are additionally auto-joined to their shift `property:<id>` from the JWT claim. The console `/live/admin` socket auto-joins the `admin` room. No driver JWT / session is required for the guest socket — it is public by design. The guest socket URL is configured via `NEXT_PUBLIC_WS_URL`; if the env var is not set, the mobile app disables WebSocket and relies on 15-second polling.
