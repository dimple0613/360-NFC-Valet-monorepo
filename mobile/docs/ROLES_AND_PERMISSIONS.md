...
| Staff validation (property-level) | `POST /api/public/offer/validate` with `propertyId` | anyone holding the correct property staff code |
| Staff validation (offer-level) | `POST /api/public/offer/validate` with `offerId` | anyone holding the correct offer staff code (currently hidden in UI) |
